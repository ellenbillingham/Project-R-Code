...
for (b in X) {
  ...
  #out$time = NULL
  Y = c(Y, out$time[out$I==max(out$I)])
}
...
n = nrow(X)
Y = Y + rnorm(n, sd=0.5) # how to choose this value of sd???
D = distance(X)
...
outg <- optim(c(1.375, 0.01*var(Y)), nl, gradnl, method="L-BFGS-B", 
              lower=eps, upper=c(2.875, var(Y)), D=D, Y=Y)
...
for (b in XX) {
  ...
  out = as.data.frame(out)
  #out$time = NULL
  trueY = c(trueY, out$time[out$I==max(out$I)])
}
...
matplot(XX, t(YY), type="l", lty=1, col="gray", xlab=expression(beta), ylab="Time of peak infections", ylim=c(-15, 40))
